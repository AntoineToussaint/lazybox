...
import { DESKTOP_PROTOCOL_VERSION } from "./generated";
...
describe("generated desktop compatibility fixture", () => {
  ...
  it("covers every exported command and event variant", () => {
    expect(new Set(fixture.commands.map(variantTag)).size).toBe(64);
    expect(new Set(fixture.events.map(variantTag)).size).toBe(64);
  });
});
