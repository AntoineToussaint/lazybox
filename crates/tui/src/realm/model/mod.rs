...
impl<T: TerminalAdapter> Model<T> {
    ...
    pub(super) fn refresh_usage_limit_alert(&mut self) {
        ...
        if was == 0 || mine_up {
            ...
            // standing banner is the one persistent surface that can name
            // them, so it does. (Parked-only raises no banner: the gate above
            // is still the blocked count alone, so a self-resolving wait never
            // escalates to a sticky banner on its own.)
