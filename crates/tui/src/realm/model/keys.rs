...
impl<T: TerminalAdapter> Model<T> {
    ...
    pub fn handle_mouse(&mut self, m: crossterm::event::MouseEvent) {
        ...
        // Account for a hidden Activity pane: when hidden, `right_top`
        // is zero-height so clicks there route to the terminal stack
        // (and the horizontal splitter disappears).
        let (sidebar_rect, right_top_rect, right_bottom_rect) =
