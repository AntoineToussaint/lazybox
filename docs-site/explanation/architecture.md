...
[remote over SSH](../how-to/remote-over-ssh.md) works.
...
The reactive behaviour described in the [mental model](mental-model.md) is built
on an in-process broadcast bus inside the daemon. **Providers produce** events
as they poll upstream; **subscribers consume** them — the TUI to render, the
JSON API gateway to forward. This producer/subscriber decoupling is why adding a
new source (or a new consumer) doesn't require touching the others.
