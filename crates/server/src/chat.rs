...
fn format_terminal_status(
    ...
    let state_label = match agent_states.get(&tid).copied() {
        ...
        Some(AgentState::AwaitingReset) => "💤 awaiting reset",
        Some(AgentState::Stalled) => "⚠️ stopped on error",
...
/// working, `✓` if done, `✗` if the process exited, `·` if idle or
/// untracked.
