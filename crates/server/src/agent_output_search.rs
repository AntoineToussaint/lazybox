...
/// back. The client then ANDed `agent:error agent:deadlock`, found no
/// evidence for the second, and excluded a workspace whose output
/// contained both — a silent miss with nothing on screen to explain it.
/// Per-needle buckets give every term its own room.
