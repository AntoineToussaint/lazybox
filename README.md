...
- **🎚️ GitHub-controlled compute** — a `model:s` / `model:m` / `model:l` / `model:xl` label (or an `@model:<tier>` task-body marker) names a tier of the target agent's model menu, so GitHub can choose model and reasoning effort before work starts. The label says what it does: capability, not urgency — ordering labels stay ordering labels.
...
- **Model tiers** — a GitHub `model:<tier>` label (or `@model:<tier>` task-body marker) chooses the target agent's configured model and reasoning-effort arguments at spawn. The token matches a tier by alias (`model:l`), label (`model:opus`) or pinned id (`model:claude-opus-5`). `w S` / `w M` / `w L` is the direct in-TUI override; Claude ships a Haiku/Sonnet/Opus/Fable menu (`claude-haiku-4-5` / `claude-sonnet-5` / `claude-opus-5` / `claude-fable-5-1`, Opus the default and Fable reachable only by name) and other agents configure theirs under `agents.<id>.models`, where a declared tier overlays the built-in menu by alias (`replace: true` takes the block as the whole menu instead). The picked tier rides a `◆ Opus` tab badge. The older `best` / `high` / `medium` / `low` labels still work, but they are capability aliases configured under `models.capability`, not task priorities.
...
- **Epics with live execution order** — the `E` leader opens blockers, roles, the dependency graph, merge order, and independent dispatch/review/merge automation for a tracker parent and its cross-repo members. `E j` jumps directly to the next blocked member instead of asking an agent to reconstruct status. Follow the [cross-repo epic guide](https://lazybox.ai/docs/how-to/run-cross-repo-epic/).
- **Automation policies** — `g p` opens the unified policies menu: merge-on-green, per-session auto-fix, and GitHub-native auto-merge, each toggled in place. Armed policies show as row pills (`ARM`, `FIX`). Arming merge-on-green also turns on GitHub's own auto-merge — but only where the base's required checks and reviews provably cover lazybox's own gate, since GitHub waits on required checks alone — so the PR lands even with lazybox closed (`merge_on_green.github_native`).
- **Cross-agent bus** — every spawned Claude session is wired to a daemon-hosted MCP server: sessions can inspect and message siblings, share durable notes, ask questions with tracked replies, read the live epic snapshot, declare blockers, and let a Coordinator spawn work on an existing tracker record. `task_status` answers "is anyone working on `owner/repo#N`?" — from a shell, `lazybox task status <ref>`. Agents are briefed on the tools at session start, so "check the blackboard first" just works. See [cross-agent coordination](docs/features/terminals-and-agents.md#cross-agent-coordination-mcp-bus).
