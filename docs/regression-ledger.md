...
| `crates/server/src/pty.rs::resize_announces_the_new_size_in_stream_order` | #1547 — the daemon is the PTY-size authority: a resize is announced on the output stream as an empty chunk with the new size, sequenced before the child's next bytes, and later chunks are stamped with it (unit, real PTY child) |
| `crates/server/src/pty.rs::size_spans_shift_with_the_dropped_partial_line` | #1547 — a wrapped ring's replay reports its size spans relative to the boundary-clean replay it hands out, so a reconnect parses each run at the size it was produced at |
| `crates/server/src/spawn_handler.rs::pump_publishes_resize_announcements_and_stamps_output` | #1547 — the pump forwards a resize announcement as stream-ordered `TerminalOutput` without running the agent-state detectors on it |
| `crates/tui/src/components/terminal_stack.rs::hidden_backlog_parses_at_the_size_it_was_stamped_with` | #1547 (#909's shape) — a hidden terminal's Ink repaints parse at the PTY size the daemon stamped them with, whatever pane the terminal reappears in: the repainted top line is in scrollback once, not once per repaint |
| `crates/tui/src/components/terminal_stack.rs::two_size_backlog_parses_each_run_at_its_own_size` | #1547 — a backlog produced across a PTY resize parses each run at its own size; no single client-side "producing size" could |
| `crates/tui/src/components/terminal_stack.rs::visible_terminal_parses_in_flight_repaints_at_the_pty_size` | #1547 — a render never resizes the grid; repaints still in flight after a pane shrink parse at the PTY's size until the daemon announces the new one |
| `crates/tui/src/components/terminal_stack.rs::reconnect_replay_parses_each_span_at_its_size` | #1547 (#1405's shape) — a reconnect replay straddling a resize rebuilds the grid a live client built |
| `crates/tui/src/realm/model/tests.rs::a_size_change_ends_the_run` | #1547 — the drain coalescer never merges chunks stamped with different PTY sizes into one feed |
| `crates/tui/src/realm/model/tests.rs::a_resize_announcement_is_never_merged_with_same_size_output` | #1547 — the drain coalescer keeps an empty-bytes resize announcement as its own event; merged into same-size output it vanished, the client's resize request stayed "unanswered", and two clients on one terminal re-requested against each other forever |
| `crates/server/src/pty.rs::size_log_is_capped_by_folding_its_oldest_changes` | #1547 — the ring's PTY-size log is bounded (`SIZE_LOG_CAP`) independent of ring turnover, so a resize per few bytes of output can't grow every snapshot/resync without limit |
| `crates/server/src/spawn_handler.rs::zero_sized_resize_never_reaches_the_backend` | #1547 — a `Resize` with a zero dimension is dropped at the command boundary, so `0 × 0` (the client's "size unknown" value) is never a stamp the daemon produces |
