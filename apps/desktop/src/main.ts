...
/** Mount one desktop client instance. All state and effects live until dispose. */
export function init(root: Document | HTMLElement = document): DesktopApp {
  const rootDocument =
  ...
  const jumpWorkspaceButton = element<HTMLButtonElement>(
    "jump-workspace-button",
  );
  ...
  const terminalStreamNotice = element<HTMLDivElement>(
    "terminal-stream-notice",
  );
  ...
  const workspaceGrid = root.querySelector<HTMLElement>(".workspace-grid")!;
  const rightPane = root.querySelector<HTMLElement>(".right-pane")!;
  ...
  const messagesButton = element<HTMLButtonElement>("messages-button");
  const messagesCount = element<HTMLSpanElement>("messages-count");
  const messagesDialog = element<HTMLDialogElement>("messages-dialog");
  const messagesList = element<HTMLDivElement>("messages-list");
  const messagesClose = element<HTMLButtonElement>("messages-close");
  ...
  const githubAuthMessage = element<HTMLParagraphElement>(
    "github-auth-message",
  );
  ...
  const discoverReposButton = element<HTMLButtonElement>(
  ...
  const notificationsEnabled = element<HTMLInputElement>(
    "notifications-enabled",
  );
  const notificationPreviews = element<HTMLInputElement>(
  ...
  const newWorkspaceProject = element<HTMLSelectElement>(
  ...
  const newWorkspaceError = element<HTMLParagraphElement>(
    "new-workspace-error",
  );
  ...
  let pendingFilterSet: Set<Filter> | null = null;
  ...
  const policyMutations = new MutationLedger<string>();
  const requestGenerations = new RequestGenerations();
  const pendingLaunches = new Set<string>();
  const pendingActions: Array<{ workspaceKey: string; action: string }> = [];
  ...
  newWorkspaceCancel.addEventListener(
    "click",
    () => newWorkspaceDialog.close(),
    { signal: lifecycle.signal },
  );
  newWorkspaceForm.addEventListener(
    "submit",
    (event) => {
  ...
  // #993's contextual-work / broadcast / jump controls, wired with the
  // lifecycle signal (#974) so dispose() tears them down like every other
  // listener and a reinit leaves no duplicates.
  ...
  jumpFailingButton.addEventListener(
    "click",
  ...
  broadcastSnippet.addEventListener(
    "click",
    () => {
  ...
  autoMergeToggle.addEventListener(
    "change",
    () => {
  ...
  trackMainToggle.addEventListener(
    "change",
    () => {
  ...
  autoFixCiSelect.addEventListener(
    "change",
  ...
  notesBody.addEventListener(
    "input",
    () => {
      if (selectedKey !== null) {
        saveNotesDraft(selectedKey, notesBody.value);
        notesBody.removeAttribute("data-remote-conflict");
      }
    },
    { signal: lifecycle.signal },
  );
  ...
  notificationPermissionButton.addEventListener(
    "click",
    () => {
  ...
  updateDismiss.addEventListener(
    "click",
    () => {
      if (dismissedUpdateKey !== null) {
        localStorage.setItem(
          `lazybox.updateDismissed.${dismissedUpdateKey}`,
          "1",
        );
      }
  ...
  messagesClose.addEventListener("click", () => messagesDialog.close(), {
    signal: lifecycle.signal,
  });
  snippetButton.addEventListener("click", () => void openSnippetPicker(), {
    signal: lifecycle.signal,
  });
  terminalStreamHealth.addEventListener(
    "click",
    () => {
  ...
  terminalStreamDismiss.addEventListener(
    "click",
    () => {
  ...
  defaultAgentSelect.addEventListener(
    "change",
  ...
  // Reflect repo-availability synchronously before the async boot: the "New
  // workspace" button must start disabled (no repositories loaded yet) rather
  // than briefly clickable-but-inert, and this makes the state a reliable gate
  // for boot completion instead of racing it (#974 init()/dispose lifecycle).
  ...
  return {
    dispose(): void {
      ...
      if (!previewMode && eventChannel !== null) {
        void invoke("unsubscribe_events", { controllerId }).catch((error) => {
          console.warn("lazybox desktop stream cleanup failed", error);
        });
      }
      if (searchTimer !== undefined) {
        window.clearTimeout(searchTimer);
      ...
      requestGenerations.dispose();
      notesDrafts.clear();
      replySubmitting.clear();
  ...
  async function boot(): Promise<void> {
    // Bounded idle refresh of relative-time labels (#974), independent of the
    // event-driven renders.
    relativeTimeTimer = window.setInterval(updateRelativeTimes, 30_000);
    if (previewMode) {
  ...
  function applyBuildInfo(info: DesktopBuildInfo | undefined): void {
    ...
    const dismissed =
      info.update !== null &&
      localStorage.getItem(`lazybox.updateDismissed.${info.update.key}`) ===
        "1";
  ...
  async function refreshInbox(
    requestProviderRefresh: boolean,
  ): Promise<boolean> {
    const generation = requestGenerations.begin("refresh");
    const baselineRevision = workspaceRevision;
    inboxLoading = true;
    inboxError = null;
    renderInbox();
    try {
      ...
      if (disposed || !requestGenerations.isCurrent("refresh", generation)) {
        return false;
      }
      if (workspaceRevision === baselineRevision) {
        workspaces = new Map(
      ...
      if (initial.warnings.length > 0) {
        setStatus(
          initial.warnings[0] ?? "Some workspaces could not be decoded.",
        );
      }
  ...
  function handleStreamMessage(message: DesktopStreamMessage): void {
    ...
    if (message.type === "Inbox") {
      // The desktop's own Tauri host is the sole producer of this view and
      // always stamps a monotonic `revision`. Gate strictly on it; do NOT
      // synthesize an always-newer value on a missing field, which silently
      // reverts to racy arrival-order ordering — the exact defect this gate
      // exists to prevent (#974).
      const revision = message.payload.revision;
      if (revision <= inboxRevision) {
        return;
      }
      ...
      inboxLoading = false;
      const authoritativeFilters = new Set(
        activeFilters(message.payload.filter_menu).map((item) => item.filter),
      );
      if (
        pendingFilterSet === null ||
        sameSet(pendingFilterSet, authoritativeFilters)
      ) {
        activeFilterSet = authoritativeFilters;
        pendingFilterSet = null;
      }
  ...
  function handleEvent(event: LazyboxEvent): void {
    ...
    const terminalChanged =
      ...
      // `TerminalReplaced` (#973) is a structural terminal change that needs a
      // re-sync, like spawn/exit. `AgentState` drives the live asking/running/
      // ready row badge (#975), which rides `workspaceRowSig`, so it must
      // re-render — but the keyed reconciler (#877) rebuilds only the row whose
      // badge changed and reuses the rest, so an agent-state burst never
      // remounts the list (#974).
    ...
    if ("Snapshot" in event) {
      ...
      setStatus(
        `${event.ProviderError.source}: ${event.ProviderError.message}`,
      );
    } else if ("CommandRejected" in event) {
      const rejectedPolicies = policyMutations.rejectCommand(
        event.CommandRejected.command,
        event.CommandRejected.message,
      );
      for (const rejected of rejectedPolicies) {
        pendingPolicies.delete(
          policyKey(rejected.workspaceKey, rejected.operation),
        );
      }
      const actionIndex = pendingActions.findIndex(
        (action) => action.action === event.CommandRejected.command,
      );
      if (rejectedPolicies.length === 0 && actionIndex !== -1) {
        const [action] = pendingActions.splice(actionIndex, 1);
        if (action !== undefined) {
          policyMutations.recordOutcome(
            action.workspaceKey,
            action.action,
            "rejected",
            event.CommandRejected.message,
      ...
      const actionIndex = pendingActions.findIndex(
        (action) =>
          action.workspaceKey === event.WorkspaceActionOutcome.workspace_key,
      );
      const [action] =
        actionIndex === -1 ? [] : pendingActions.splice(actionIndex, 1);
      policyMutations.recordOutcome(
        event.WorkspaceActionOutcome.workspace_key,
        action?.action ?? "workspace-action",
        event.WorkspaceActionOutcome.ok ? "confirmed" : "rejected",
        event.WorkspaceActionOutcome.message,
        "",
      );
      renderMessages();
  ...
  async function readTerminalData(): Promise<void> {
    while (!previewMode && !disposed) {
      ...
      try {
        ...
        // The backend releases a parked reader when this controller is
        // superseded (dispose → unsubscribe, or a re-init's subscribe), so the
        // await resolves promptly with an error. Bail before touching disposed
        // state so the loop cannot outlive its instance or steal frames from a
        // newer one (#974); otherwise it's a genuine disconnect (#973).
        if (disposed) {
          break;
        }
  ...
  async function applyFilters(filters: Filter[]): Promise<void> {
    ...
    const generation = requestGenerations.begin("filters");
    try {
      await invoke("set_filters", { filters, generation, controllerId });
    } catch (error) {
      if (requestGenerations.isCurrent("filters", generation)) {
        pendingFilterSet = null;
        activeFilterSet = new Set(
          activeFilters(currentFilterMenu()).map((item) => item.filter),
        );
        renderFilterControls();
  ...
  function toggleFilter(filter: Filter): void {
    ...
    pendingFilterSet = new Set(activeFilterSet);
    renderFilterControls();
  ...
  async function applySearch(query: string): Promise<void> {
    ...
    const generation = requestGenerations.begin("search");
    try {
      await invoke("set_search", { query, generation, controllerId });
    } catch (error) {
      if (requestGenerations.isCurrent("search", generation)) {
        setStatus(String(error));
      }
    }
  }
  ...
  function renderRepoHeader(repo: string): HTMLButtonElement {
    ...
    meta.textContent =
      attention > 0 ? `${active} · ${attention} ⚑` : `${active}`;
  ...
  function renderWorkspace(): void {
    ...
    shellButton.textContent =
      terminalForWorkspace(workspace.key, "shell") === undefined
        ? "Shell"
        : "Open shell";
    shellButton.disabled = pendingLaunches.has(
      launchKey(workspace.key, "shell"),
    );

    activityCount.textContent = String(workspace.activity.length);
    workActivityButton.disabled =
      selectedActivityIndices(workspace).length === 0;
  ...
  function renderPolicyToggle(
    ...
    if (requested === String(committed)) {
      ...
      policyMutations.confirmValue(workspaceKey, field, String(committed));
      renderMessages();
      toggle.checked = committed;
    } else {
  ...
  function renderAutoFixPolicies(workspace: Workspace): void {
    ...
    if (requested === undefined || requested === committed) {
      if (requested === committed) {
        pendingPolicies.delete(key);
        policyMutations.confirmValue(workspace.key, "auto_fix", committed);
        renderMessages();
      }
  ...
  /**
   * Remote notes replace a clean editor. A dirty editor keeps the local draft
   * and exposes a conflict marker until the user saves or edits again.
   */
  function synchronizeSelectedNotes(event: LazyboxEvent): void {
    if (selectedKey === null) {
      return;
    }
    const incoming =
      "WorkspaceUpserted" in event
        ? event.WorkspaceUpserted
        : "Snapshot" in event
          ? event.Snapshot.workspaces.find(
              (workspace) => workspace.key === selectedKey,
            )
          : undefined;
    if (incoming === undefined || incoming.key !== selectedKey) {
      return;
    }
    const previous = savedNotes(selectedKey);
    const local = notesBody.value;
    const dirty = notesDrafts.has(selectedKey) || local !== previous;
    if (!dirty || local === incoming.notes) {
      notesDrafts.delete(selectedKey);
      notesBody.value = incoming.notes;
      notesBody.removeAttribute("data-remote-conflict");
      notesBody.removeAttribute("title");
    } else if (incoming.notes !== previous) {
      notesDrafts.set(selectedKey, local);
      notesBody.dataset.remoteConflict = "true";
      notesBody.title =
        "Notes changed remotely; your unsaved draft is preserved.";
      setStatus("Notes changed remotely; your unsaved draft is preserved.");
    }
  }
  ...
  function layoutTiles(): void {
    ...
    wanted.forEach((record, index) => {
      ...
      if (terminalTabs.children[index] !== live.tab) {
        terminalTabs.insertBefore(
          live.tab,
          terminalTabs.children[index] ?? null,
        );
      }
  ...
  function initColumnSplitter(): void {
    ...
    columnSplitter.addEventListener(
      "mousedown",
      (event) => {
        event.preventDefault();
        ...
        const onUp = () => {
          ...
          const current =
            workspaceGrid.style.getPropertyValue("--sidebar-width");
          if (current.endsWith("px")) {
        ...
        window.addEventListener("mouseup", onUp, { signal: lifecycle.signal });
      },
      { signal: lifecycle.signal },
    );
  }
  ...
  function initActivitySplitter(): void {
    ...
    rightPaneSplitter.addEventListener(
      "mousedown",
      (event) => {
        event.preventDefault();
  ...
  async function openSnippetPicker(): Promise<void> {
    ...
    snippetQuery = "";
    snippetFilter.value = "";
    const generation = requestGenerations.begin("snippets");
    const view = await fetchSnippetView("");
    if (
      view === null ||
      disposed ||
      !requestGenerations.isCurrent("snippets", generation)
    ) {
      return;
    }
  ...
  function onSnippetDialogClose(): void {
    requestGenerations.cancel("snippets");
  ...
  async function requestWorkspaceDiff(workspaceKey: string): Promise<void> {
    ...
    if (
      !(await sendCommand(inspectWorkspaceDiffCommand(workspaceKey, target)))
    ) {
      pendingDiffKey = null;
    }
  }
  ...
  function showWorkspaceDiff(
    ...
    diffTitle.textContent =
      task !== null
        ? `Worktree diff · ${taskReference(task)}`
        : "Worktree diff";
    if (!diffDialog.open) {
  ...
  async function runPolicyMutation(
    ...
    const key = policyKey(workspaceKey, field);
    const previous =
      pendingPolicies.get(key) ?? committedPolicyValue(workspaceKey, field);
    policyMutations.begin(
      workspaceKey,
      field,
      commandName(command),
      previous,
    ...
    if (!ok && pendingPolicies.get(key) === requested) {
      pendingPolicies.delete(key);
      policyMutations.reject(
        workspaceKey,
        field,
        statusMessage.textContent ?? "Command failed.",
      );
      renderMessages();
      renderWorkspace();
    }
  ...
  function committedPolicyValue(workspaceKey: string, field: string): string {
    const workspace = workspaces.get(workspaceKey);
    if (workspace === undefined) {
      return "";
    }
    if (field === "auto_merge") {
      return String(workspace.auto_merge_on_green);
    }
    if (field === "track_main") {
      return String(workspace.track_main);
    }
    return `${workspace.policies.auto_fix_ci}\0${workspace.policies.auto_fix_conflict}`;
  }

  function commandName(command: LazyboxCommand): string {
    return typeof command === "string"
      ? command
      : (Object.keys(command)[0] ?? "command");
  }
  ...
  async function saveNotes(): Promise<void> {
    ...
    try {
      ...
      if (!ok) {
        saveNotesDraft(workspaceKey, notes);
      }
    } finally {
  ...
  async function sendCommand(command: LazyboxCommand): Promise<boolean> {
    ...
    try {
      ...
      if (context !== null) {
        const index = pendingActions.indexOf(context);
        if (index !== -1) {
  ...
  function commandContext(
    command: LazyboxCommand,
  ): { workspaceKey: string; action: string } | null {
    ...
    if (
      action === undefined ||
      !["MergePr", "UpdateBranch", "CloseIssue", "DeleteOrClose"].includes(
        action,
      )
    ) {
      return null;
    }
    const payload = (command as unknown as Record<string, unknown>)[action];
    if (
    ...
    return { workspaceKey: payload.session_key, action };
  }
  ...
  function renderThemeList(): void {
    ...
    for (const [index, theme] of availableThemes.entries()) {
      ...
      swatch.setAttribute(
        "aria-checked",
        String(theme.name === effectiveTheme),
      );
  ...
  function promptText(options: {
    ...
    return new Promise((resolve) => {
      inputDialog.addEventListener(
        ...
        () =>
          resolve(
            inputDialog.returnValue === "submit" ? inputField.value : null,
          ),
        { once: true },
      );
    });
  ...
  function renderMessages(): void {
    const records = [...policyMutations.history()].reverse();
    messagesCount.textContent = String(records.length);
    const entries = records.map(
      (record): RowEntry => ({
        key: `message:${record.id}`,
        sig: `${record.state}\0${record.message}`,
        build: () => {
          const node = document.createElement("article");
          node.className = "message-record";
          node.dataset.state = record.state;
          const heading = document.createElement("strong");
          heading.textContent = `${record.workspaceKey} · ${record.operation} · ${record.state}`;
          const message = document.createElement("p");
          message.textContent = record.message;
          node.replaceChildren(heading, message);
          return node;
        },
      }),
    );
    reconcileList(messagesList, entries);
  }
  ...
  function updateRelativeTimes(): void {
    for (const time of root.querySelectorAll<HTMLTimeElement>(
      "time[datetime]",
    )) {
