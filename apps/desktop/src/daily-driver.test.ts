...
describe("desktop daily-driver parity", () => {
  ...
  it("preserves marks and broadcasts across agent, shell, and session-less targets", async () => {
    const workspaces = [workspace("agent"), workspace("shell"), workspace("new")];
    const terminals = [terminal("agent", 7, { Agent: "codex" }), terminal("shell", 8, "Shell")];
    await boot(workspaces);
    stream().onmessage({ type: "Frame", payload: { Snapshot: { workspaces, terminals, recent_snippets: [] } } });
    stream().onmessage(inbox(["agent", "shell", "new"]));
    await vi.waitFor(() => expect(document.querySelectorAll(".workspace-row")).toHaveLength(3));

    for (const key of ["agent", "shell", "new"]) {
      row(key).querySelector<HTMLElement>(".workspace-mark-toggle")?.click();
    }
    stream().onmessage({ type: "Frame", payload: { WorkspaceUpserted: workspace("agent") } });
    stream().onmessage(inbox(["agent", "shell", "new"]));
    ...
    form("broadcast-form").dispatchEvent(new Event("submit", { bubbles: true, cancelable: true }));
    ...
    expect(element("status-message").textContent).toContain("agent, shell, new");
  });

  it("reports a target removed mid-broadcast instead of silently dropping it", async () => {
    const workspaces = [workspace("a"), workspace("b")];
    const terminals = [terminal("a", 1, { Agent: "codex" }), terminal("b", 2, { Agent: "codex" })];
    await boot(workspaces);
    stream().onmessage({ type: "Frame", payload: { Snapshot: { workspaces, terminals, recent_snippets: [] } } });
    stream().onmessage(inbox(["a", "b"]));
    await vi.waitFor(() => expect(document.querySelectorAll(".workspace-row")).toHaveLength(2));
    ...
    await vi.waitFor(() =>
      expect(element("status-message").textContent).toContain("no longer available"),
    );
  ...
  it("paginates a large activity feed explicitly instead of rendering all rows", async () => {
    ...
    target.activity = Array.from({ length: 120 }, (_, index) => activity(index));
    await boot([target]);
    stream().onmessage({ type: "Frame", payload: { Snapshot: { workspaces: [target], terminals: [], recent_snippets: [] } } });
    stream().onmessage(inbox(["big"]));
    // First page is bounded; the rest are behind an explicit control.
    await vi.waitFor(() => expect(document.querySelectorAll(".activity-card")).toHaveLength(50));
    expect(element("activity-count").textContent).toBe("120");
    const more = document.querySelector<HTMLButtonElement>(".activity-show-more")!;
    expect(more.textContent).toContain("70");

    more.click();
    await vi.waitFor(() => expect(document.querySelectorAll(".activity-card")).toHaveLength(100));
    document.querySelector<HTMLButtonElement>(".activity-show-more")!.click();
    await vi.waitFor(() => expect(document.querySelectorAll(".activity-card")).toHaveLength(120));
  ...
  it("renders the full activity feed, works on stable selection, and confirms cleanup", async () => {
    ...
    await boot([target]);
    stream().onmessage({ type: "Frame", payload: { Snapshot: { workspaces: [target], terminals: [live], recent_snippets: [] } } });
    stream().onmessage(inbox(["activity"]));
    await vi.waitFor(() => expect(document.querySelectorAll(".activity-card")).toHaveLength(35));

    const last = document.querySelectorAll<HTMLElement>(".activity-card")[34]!;
    last.querySelector<HTMLButtonElement>(".activity-card-actions button")?.click();
  ...
  it("surfaces the asking badge and jump target when an agent needs input", async () => {
    ...
    await vi.waitFor(() => expect(document.querySelectorAll(".workspace-row")).toHaveLength(1));
    ...
    stream().onmessage({
      type: "Frame",
      payload: { AgentState: { session_key: "asking", terminal_id: 11, state: "InputNeeded" } },
    });
    await vi.waitFor(() => expect(row("asking").textContent).toContain("asking"));

    // Jump-to-asking locates it instead of reporting "No agent is asking."
    button("jump-asking-button").click();
    expect(element("status-message").textContent).not.toContain("No agent is asking");
  });
  ...
  it("does not offer cleanup merely because the last terminal exited", async () => {
    ...
    stream().onmessage({
      type: "Frame",
      payload: { TerminalExited: { terminal_id: 21, exit_code: 0, last_output: null } },
    });
    ...
    expect(commandCalls()).not.toContainEqual({ KeepWorkspace: { session_key: "healthy" } });
  });
  ...
  harness.invoke.mockImplementation((command: string, args?: unknown) => {
    if (command === "desktop_setup_state") {
      return Promise.resolve({
        ...
        agents: [{ id: "codex", label: "Codex", available: true, models: [], default_tier: null }],
        default_agent: "codex",
    ...
    if (command === "list_workspaces") return Promise.resolve({ workspaces, warnings: [] });
    if (command === "read_terminal_data") return new Promise<Uint8Array>(() => {});
...
type WorkspaceFixture = Record<string, any>;

function workspace(key: string): WorkspaceFixture {
  const template = structuredClone((fixture.events[0] as any).Snapshot.workspaces[0]);
  template.key = key;
  template.name = key;
  template.branch = key;
  template.sessions = [];
...
function terminal(sessionKey: string, terminalId: number, kind: unknown): Record<string, unknown> {
  return {
...
function inbox(keys: string[]): Record<string, unknown> {
  return {
    ...
    payload: {
      outcome: {
        visible: [{ RepoHeader: "o/r" }, ...keys.map((key) => ({ Workspace: key }))],
...
function row(key: string): HTMLElement {
  return document.querySelector<HTMLElement>(`.workspace-row[data-key="${key}"]`)!;
}
