...
is available. It never updates itself: Homebrew installs use
`brew upgrade lazybox`, while source builds use
`git pull --ff-only && cargo build --release` before restarting.
