//! Shared scaffolding for `lazybox-git-ops`'s integration binaries.
//!
//! `mod common;` in a `tests/*.rs` links in a before-main `#[ctor]` that
//! points every git this binary spawns at a sandbox global config —
//! no signing setup, no detached background maintenance (#1751), plus a guard test that fails *this* binary if the
//! redirect ever stops working. `crates/core/tests/test_isolation.rs`
//! requires it in every integration binary of a crate that spawns git.
//!
