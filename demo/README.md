...
- [`golden.tape`](golden.tape) — the CI snapshot. Opens the `?` help
  overlay only, which is static lazybox chrome (no live shell), so its text
...
`lazybox.*` assets are committed too, but they are **not** diffed in CI:
`lazybox.tape` spawns a shell, whose prompt carries a random tempdir
...
`lazybox` binary renders the same `golden.ascii` every time — which is what
...
(it must still boot `lazybox --test`), but it is not diffed.
