...
5. **No real subprocesses in unit tests.** No real `claude` / `sh` / `curl` / `tmux` invocations outside the integration tier. Mock at the trait boundary (`Agent`, `SessionBackend`, `CredentialProvider`) or `#[ignore]`-gate. The deliberate exceptions are `crates/git-ops` (real git against local upstreams) and the `e2e_*` integration tier (`crates/server/tests/e2e_real_paths.rs`, #410): real git / tmux / agent binaries asserting user-visible outcomes, with a wider nextest timeout keyed on the `e2e_` name prefix and a nightly lane (`.github/workflows/nightly.yml`) where missing prerequisites fail loudly instead of skipping. A recurring bug's regression test belongs in the shape that actually broke — see `docs/regression-ledger.md`, which is machine-checked by `crates/core/tests/regression_ledger.rs`.
