...
| `crates/agents/src/agent.rs::claude_seeds_inline_renderer_for_scrollback` | real Claude's full-screen renderer repaints a bounded viewport and leaves tmux history empty even when the pane rejects alternate-screen requests; Claude spawns select its supported inline renderer so transcript lines reach scrollback |
