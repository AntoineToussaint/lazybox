...
contract arrives. A row is dropped when its epic is *deleted* — not when the
epic stops listing the producer as a member, since a prune that fires a beat
too eagerly re-creates the very failure the latch exists to stop — and an
archived epic keeps its rows, so unarchiving does not demand every interface be
published again.
...
6. **Contract granularity** — a `Contract` edge is satisfied by the *existence*
   ...
   the producer publishes a strictly newer note) and the interface text, so a
