...
<!--
TODO(assets): the hosted hero video below is a GitHub user-attachments upload and
is NOT regenerated by the demo pipeline — re-upload a fresh capture of
`demo/lazybox.mp4` and replace the `src=` URL. The poster/fallback now point at
`demo/lazybox.{png,gif}`, which ARE the pipeline's canonical hero outputs (see
demo/README.md), not the older `demo/hero.*` files. Regenerate the fleet demo
with `vhs demo/lazybox.tape` (see demo/README.md → "Regenerating locally").
-->
<div align="center">

<video src="https://github.com/user-attachments/assets/c3f11255-5cba-4904-8c58-299ebfcccef6" poster="demo/lazybox.png" controls muted autoplay loop playsinline width="900">
  <img src="demo/lazybox.gif" alt="lazybox: the inbox on the left listing live PRs and issues across repos, an opened workspace with description, activity, and embedded agent terminals on the right" />
</video>

</div>

<sub>Video not playing? Here's the [animated GIF](demo/lazybox.gif) and a [static
screenshot](demo/lazybox.png). The demo is **code, not a recording** — a fully
reproducible `--demo` fleet driven by [`demo/lazybox.tape`](demo/lazybox.tape).</sub>
